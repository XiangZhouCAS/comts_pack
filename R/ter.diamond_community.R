#' To calculate the hydrogen metabolism terminal gene abundance in microbial community (GAM,%).
#'
...
#' @param ter_db Please set the directory of diamond database (e.g.'hyddb.all.dmnd').
...
#' @param min_length Set the minimum length required for filtering reads, the default is 70 bp.
...
#' @export
#'
#' @examples diamond_community(input_reads = "your/reads/data/sample_1.fastq",result = "sample_1",threads = 40,diamond_db = "your/database/ter.dmnd",USCG_db = "your/database/Ribo_14.dmnd",min_length = 70)
ter.diamond_community <- function(input_reads,result,threads = 1,
                                  ter_db = "default",USCG_db = "default",
                                  skip_fastp = F,
                                  min_length = 70,run_seqkit = T,
                                  keep_samples = F){
  if(ter_db == "default"){
    wd1 <- paste0(find.package("comts"),"/database")
    diamond_db2 <- paste0(wd1,"/ter.dmnd.gz")
    diamond_db <- paste0(wd1,"/ter.dmnd")
    if(file.exists("ter.dmnd.gz") == T){
      system(sprintf("gunzip %s",diamond_db2))}
  }else{
    wd1 <- dirname(ter_db)
    diamond_db <- paste0(wd1,"/",basename(ter_db))}
  if(USCG_db == "default"){
