#' To calculate hydrogenase's GAM.
#'
#' @param input_reads Please set the reads to include only the forward reads if the data is paired-end (PE).
#' @param result Please specify the name for the result file.
#' @param threads Set the number of CPU threads, with the default value being 1.
#' @param hyd_db Please set the directory of single-copy genes' diamond database (e.g.'hyddb.all.dmnd').
#' @param script Please set the directory of hyd_id-name.script.
#' @param USCG_db Please specify the directory for universal single-copy genes database (e.g., 'Ribo_14.dmnd'). If you have already calculated the RPKM for these genes, you may instead specify the directory for the results (e.g., 'sample_name.USCG.hits.txt') to skip this step.
...
#' @param min_length Set the minimum length required for filtering reads, the default is 70.
...
#' @export
#'
#' @examples diamond_community(input_reads = "your/reads/data/sample_1.fastq",result = "sample_1",threads = 40,diamond_db = "your/database/hyddb.all.dmnd",USCG_db = "your/database/Ribo_14.dmnd",script = "your/database/hyd_id-name.script",min_length = 70)
hyd.diamond_community <- function(input_reads,result,threads = 1,
                                  hyd_db = "default",script = "default",
                                  USCG_db = "default",skip_fastp = F,
                                  min_length = 70,
                                  run_seqkit = T,keep_samples = F){
  if(hyd_db == "default"){
    wd1 <- paste0(find.package("comts"),"/database")
    diamond_db <- paste0(wd1,"/hyddb.all.dmnd")
  }else{
    wd1 <- dirname(hyd_db)
    diamond_db <- paste0(wd1,"/",basename(hyd_db))}
  if(USCG_db == "default"){
  ...
  if(script == "default"){
    script <- paste0(find.package("comts"),"/database/hyd_id-name.script.txt")
  }
